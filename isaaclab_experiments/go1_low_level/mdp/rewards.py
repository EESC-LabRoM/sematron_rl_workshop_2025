...
def finer_track_ang_vel_z_exp(
    ...
    is_within_bounds = (
        torch.abs(env.command_manager.get_command(command_name)[:, 2]) < vel_limit
    )
...
class FootClearenceReward(ManagerTermBase):
    ...
    def __call__(
        ...
        is_moving = 1.0 * (torch.linalg.norm(velocity, dim=1, keepdim=True) > 0.01)
        self.target_heights = (
            FootClearenceReward.compute_target_height(
